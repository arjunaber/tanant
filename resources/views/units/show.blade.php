        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center">
                    <a href="{{ route('units.index') }}" class="text-xl font-bold text-blue-600">TenantRuangan</a>
                </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <p class="font-medium 
                                    ...
                                    @endif">
                                    @if($unit->status === 'available') Tersedia
                                    @elseif($unit->status === 'occupied') Ditempati
                                    @else Perawatan
                                    @endif
                                </p>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <div class="text-right">
                                <div class="text-3xl font-bold text-blue-600">Rp {{ number_format($unit->price_per_day, 0, ',', '.') }}</div>
                        ...
                        <div class="space-y-4">
                            ...
                            <a href="{{ route('units.show', $relatedUnit->id) }}" 
                               class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition duration-200">
                                <div class="bg-gray-200 w-16 h-16 rounded flex items-center justify-center mr-4 flex-shrink-0">
                                    <span class="text-xs text-gray-500">Gambar</span>
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-medium text-gray-900">{{ $relatedUnit->name }}</h4>
                                    <p class="text-sm text-gray-600">Rp {{ number_format($relatedUnit->price_per_day, 0, ',', '.') }}/hari</p>
                                    <span class="text-xs 
                                        @if($relatedUnit->status === 'available') text-green-600
                                        @elseif($relatedUnit->status === 'occupied') text-red-600
                                        @else text-yellow-600
                                        @endif">
                                        @if($relatedUnit->status === 'available') Tersedia
                                        @elseif($relatedUnit->status === 'occupied') Ditempati
                                        @else Perawatan
                                        @endif
                                    </span>
                                </div>
