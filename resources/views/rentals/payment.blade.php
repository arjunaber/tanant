        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <nav class="mb-6">
                <ol class="flex items-center space-x-2 text-sm text-gray-600">
                    ...
                    <li><a href="{{ route('units.show', $rental->unit->id) }}" class="hover:text-blue-600">{{ $rental->unit->name }}</a></li>
                    <li class="flex items-center">
                        <svg class="w-4 h-4 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
                    </li>
                    <li><a href="{{ route('rentals.create', $rental->unit->id) }}" class="hover:text-blue-600">Peminjaman</a></li>
                    <li class="flex items-center">
                        <svg class="w-4 h-4 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    ...
                    <p class="mt-6 text-gray-500 leading-relaxed">
                        Peminjaman ruangan berhasil dibuat. Silakan selesaikan pembayaran untuk mengaktifkan peminjaman Anda.
                    </p>
                </div>
                ...
                <div class="bg-white p-6 lg:p-8">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="font-bold text-lg text-blue-600">{{ $rental->getFormattedTotalPrice() }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="text-center">
                        <button id="pay-button" class="inline-flex items-center px-6 py-3 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            payButton.addEventListener('click', function() {
                snap.pay('{{ $snapToken }}', {
                    onSuccess: function(result) {
                        ...
                        window.location.href = '{{ route("payment.success") }}?order_id={{ $rental->transaction_id }}';
                    },
                    onPending: function(result) {
                        console.log('Payment pending:', result);
                        window.location.href = '{{ route("payment.pending") }}?order_id={{ $rental->transaction_id }}';
                    },
                    onError: function(result) {
                        console.log('Payment error:', result);
                        window.location.href = '{{ route("payment.failed") }}?order_id={{ $rental->transaction_id }}';
                    },
                    onClose: function() {
