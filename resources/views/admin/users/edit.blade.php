        ...
        <form id="editUnitForm" method="POST" class="overflow-y-auto max-h-[calc(90vh-120px)]">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="category_id[]" id="edit_category" multiple
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 outline-none bg-white min-h-[120px]"
                        required>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                        @endforeach
                    </select>
                    <p class="text-sm text-gray-500 mt-1">Gunakan Ctrl + Click (atau Command + Click di Mac) untuk
                        memilih multiple kategori</p>
                </div>
...
<style>
    ...
    /* Style untuk multiple select di form edit */
    #edit_category[multiple] {
        min-height: 120px;
    }

    #edit_category[multiple] option {
    ...
    #edit_category[multiple] option:hover {
        background-color: #f0fdf4;
    }

    #edit_category[multiple] option:checked {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', () => {
                ...
                const category = button.dataset.category; // Ini sekarang string comma separated
                ...
                // Handle multiple categories selection
                if (category && categorySelect.multiple) {
                    // Reset semua selection terlebih dahulu
                    Array.from(categorySelect.options).forEach(option => {
                        option.selected = false;
                    });

                    // Split comma separated string menjadi array
                    const selectedCategoryIds = category.split(',').map(id => id.trim());
                    ...
                    // Fallback untuk single select (jika masih digunakan)
        ...
        closeModalButtons.forEach(btn => {
            if (btn) {
                btn.addEventListener('click', () => {
                    ...
                    // Reset form ketika modal ditutup (optional)
                    setTimeout(() => {
        ...
        // Prevent form submission jika tidak ada kategori yang dipilih
